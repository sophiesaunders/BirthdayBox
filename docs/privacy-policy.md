...
- No data is shared between users or devices, unless you use Apple's own iCloud/device backup features, which are governed by Apple's own privacy policy, not ours.
...
Since all data lives only on your device, deleting the app deletes all of its data. There is no separate account or server-side data to remove.
